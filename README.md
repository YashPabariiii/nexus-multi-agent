...
## Pending: Docker/k8s Live Verification

This build sandbox has no Docker installed, so `docker-compose.yml` and the `k8s/` manifests
were only syntax-validated (`docker-compose.yml`, all `k8s/**/*.yaml` parse as valid YAML), never
actually run. Everything else — all 4 sprints' business logic, the Streamlit UI, Prometheus
metrics, Grafana dashboard JSON, and the CI workflow YAML — is verified for real (22/22 pytest
tests passing against live local Postgres/Redis/Chroma, `ruff` clean, 80% coverage, all 6
dashboard pages executed via Streamlit's `AppTest` harness with zero exceptions).

Paste the prompt below to a coding agent that has Docker (and ideally `kind`/`minikube`) to close
that one remaining gap:

> This is the Nexus repo (FastAPI + Celery + LangGraph multi-agent research platform, Streamlit
> dashboard, Postgres/Redis/ChromaDB, Prometheus/Grafana). Docker Compose and Kubernetes
> manifests exist but have never been run — only syntax-checked. Verify them for real:
>
> 1. `cp .env.example .env` and fill in real `GROQ_API_KEY` / `GEMINI_API_KEY` / `TAVILY_API_KEY`
>    (ask me for these if you don't have them — without real keys, brief creation will fail fast
>    at the first LLM call, which is expected and documented, not a bug).
> 2. Run `docker-compose up --build -d` from repo root. Fix any build errors in `Dockerfile` or
>    `dashboard/Dockerfile` — this is a fresh-clone test, treat any failure as a real bug in the
>    Dockerfile/compose config, not something to work around.
> 3. Confirm all 7 services report healthy: `docker-compose ps`. Check `api` and `worker` logs
>    for startup errors (`docker-compose logs api worker`).
> 4. `curl http://localhost:8000/health/ready` → expect `{"db":"ok","redis":"ok","chroma":"ok"}`.
> 5. Register a tenant, get a JWT, `POST /v1/briefs` with `depth=quick`, poll
>    `GET /v1/briefs/{id}` until `status=complete` (or `failed` — read `error_message`).
>    If it completes: download `/v1/briefs/{id}/report/download/pdf` and confirm it's a valid
>    multi-page PDF (`pypdf.PdfReader`, `len(reader.pages) >= 5`).
> 6. Open `http://localhost:8501` (dashboard) and confirm it loads and can reach the API
>    (register/login flow at minimum; a full click-through if you can drive a browser).
> 7. Open `http://localhost:9090` (Prometheus) — confirm the `nexus-api` target is `UP`
>    (Status → Targets). Open `http://localhost:3000` (Grafana, admin/admin) — confirm all 4
>    dashboards under the "Nexus" folder load without every panel showing "no data" (some panels
>    will be sparse until more traffic runs, that's fine — check the *queries* execute, not that
>    every panel has rich data yet).
> 8. **If you have `kind` or `minikube`:** spin up a local cluster, `kubectl apply -f
>    k8s/namespace.yaml -f k8s/configmap.yaml`, create `k8s/secret.yaml` from the `.template`
>    with real values, apply the rest (`k8s/postgres/`, `k8s/redis/`, `k8s/chromadb/`,
>    `k8s/api/`, `k8s/worker/`), and confirm pods reach `Running` with passing readiness probes.
>    Flag anything that doesn't apply cleanly — image name `nexus-api:latest` in the manifests
>    assumes you've built and loaded it into the cluster (`kind load docker-image` or similar).
> 9. Report back: what worked, what broke, and the exact fix for anything broken (don't just
>    describe the problem — patch `Dockerfile`, `docker-compose.yml`, or the k8s YAML directly).
>
> Known caveat already documented above: the embedded `chromadb.PersistentClient` (shared
> volume, not a networked Chroma server) isn't safe for concurrent writes across the API's 3
> replicas / worker's 2 replicas in the k8s manifests. Don't "fix" this as part of this
> verification pass unless you have time to properly swap in Chroma's HTTP server mode — just
> confirm the single-Docker-Compose-replica case works, and note the k8s multi-replica gap in
> your report if you observe actual data corruption from it.

## Repository & Security Hygiene (Files NOT to Push)

When committing and pushing this repository to Git (GitHub/GitLab), ensure the following files and directories are **never committed**:

### 🚫 Files and Directories to Exclude

| Category | Patterns / Files | Reason |
| :--- | :--- | :--- |
| **Secrets & Keys** | `.env`, `.env.local`, `.env.*` | Contains sensitive API keys (`GROQ_API_KEY`, `GEMINI_API_KEY`, `TAVILY_API_KEY`, `LANGFUSE_SECRET_KEY`, database & mail passwords). Only commit `.env.example`. |
| **Kubernetes Secrets** | `k8s/secret.yaml`, `*.secret.yaml` | Production/staging cluster credentials. Only commit `k8s/secret.yaml.template`. |
| **Private Keys & Certs** | `*.pem`, `*.key`, `*.pfx` | SSL/TLS certificates and private keys. |
| **Vector DB / Storage** | `chroma_data/`, `.local/`, `*.sqlite3`, `*.db` | Local ChromaDB vector data, binary index files (`*.bin`), and SQLite databases. |
| **Python Virtualenvs & Bytecode** | `.venv/`, `dashboard/.venv/`, `__pycache__/`, `*.pyc`, `*.egg-info/`, `build/`, `dist/` | Environment and compiled build artifacts. |
| **Test & Lint Caches** | `.pytest_cache/`, `.ruff_cache/`, `.mypy_cache/`, `.coverage`, `htmlcov/` | Test runner and linter cache directories. |
| **Logs & Scratches** | `*.log`, `logs/`, `.context/`, `evals/results/*.json`, `RESUME_TODO.md` | Session scratchpad, logs, and evaluation result dumps. |
| **OS & IDE Metadata** | `.DS_Store`, `Thumbs.db`, `.vscode/`, `.idea/` | Operating system metadata and local editor configurations. |

### ✅ Safe to Commit

- [`.env.example`](.env.example) — Template showing required configuration variables with dummy values.
- [`k8s/secret.yaml.template`](k8s/secret.yaml.template) — Template with dummy placeholders.
- [`docker-compose.yml`](docker-compose.yml), [`requirements.txt`](requirements.txt), and all core application code in `app/`, `dashboard/`, `k8s/`, `tests/`, and `evals/`.

### 🛡️ Quick Pre-Push Check

Before running `git push`, run:

```bash
# Verify what is untracked or staged:
git status

# Ensure no secret files or large binary databases are staged:
